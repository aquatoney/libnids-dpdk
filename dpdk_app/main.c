...
static void l2fwd_main_loop(void)
{
    ...
    uint64_t send_pkt_num = 0;
    uint64_t send_byte_num = 0;
    uint64_t rcv_pkt_num = 0;
    uint64_t rcv_byte_num = 0;
        ...
        if (unlikely(diff_tsc > drain_tsc))
        {
            ...
            if (timer_period > 0)
            {
                ...
                if (unlikely(timer_tsc >= timer_period))
                {
                    ...
                    {
                        // print_stats();
                        double send_avg_mpps = (double)send_pkt_num * rte_get_tsc_hz() / (timer_tsc * 1000 * 1000);
                        double send_avg_mbps = (double)send_byte_num * rte_get_tsc_hz() * 8 / (timer_tsc * 1000 * 1000);
                        double rcv_avg_mpps = (double)rcv_pkt_num * rte_get_tsc_hz() / (timer_tsc * 1000 * 1000);
                        double rcv_avg_mbps = (double)rcv_byte_num * rte_get_tsc_hz() * 8 / (timer_tsc * 1000 * 1000);
                        printf("sending  : avg pps: %lfMpps, avg bps %lfMbps\n", send_avg_mpps, send_avg_mbps);
                        printf("receiving: avg pps: %lfMpps, avg bps %lfMbps\n", rcv_avg_mpps, rcv_avg_mbps);
...
//                    l2fwd_mac_updating(m, dst_port);
            ...
            if (sent) {
                ...
                for (int j = 0; j < sent; j++)
                    send_byte_num += pkts_burst[j]->pkt_len;
            }
        }
    }
}
